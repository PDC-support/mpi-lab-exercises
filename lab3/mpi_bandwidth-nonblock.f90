...
integer reqs(2),stats(2,MPI_STATUS_SIZE)
...
if(rank.lt.numtasks/2) then
   do n=start,end,incr
      ...
      do i=1,rndtrps
         t1 = MPI_Wtime()
         call MPI_Isend(msgbuf,n,MPI_CHARACTER,dest,tag,MPI_COMM_WORLD,reqs(1),ierr)
         call MPI_Irecv(msgbuf,n,MPI_CHARACTER,src,tag,MPI_COMM_WORLD,reqs(2),ierr)
